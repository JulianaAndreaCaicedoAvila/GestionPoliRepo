...
namespace SongStock.Api.Controllers {
    [@Authorize]
    [ApiController]
    [Route("Codigo Plan Docente")]
    public class CodigoPlanDocenteController : BaseController {
        public CodigoPlanDocenteController(DataContext context, IHttpContextAccessor httpContextAccessor, IConfiguration configuration)
            : base(context, httpContextAccessor, configuration) { }

        [HttpPost("ed")] // /api/banco/ed => CREATE - UPDATE
        public ActionResult Edit(CodigoPlanDocente item) {
            var u = User;
            // Registro nuevo: CREATE
            if (item.Id == 0) {
                var obj = (CodigoPlanDocente)item.CopyTo(new CodigoPlanDocente());
                _db.CodigoPlanDocente?.Add(obj);
                _db.SaveChanges();
                return Ok(obj);
            } else {
                // Registro existente: EDIT
                var current = _db.CodigoPlanDocente?.First(o => o.Id == item.Id);
                if (current != null) {
                    var final = (CodigoPlanDocente)item.CopyTo(current);
                    _db.SaveChanges();
                    return Ok(final);
                }
        ...
        public ActionResult Get(int? itemId = null) {
            var item = _db.CodigoPlanDocente?.FirstOrDefault(o => o.Id == itemId);
            return Ok(item);
        }
        ...
        public ActionResult GetAll() {
            var items = _db.CodigoPlanDocente?.ToList();
            return Ok(items);
        }
        ...
        public ActionResult GetDx() {
            ...
            var items = _db.CodigoPlanDocente?.OrderBy(o => o.Codigo).ToList();
