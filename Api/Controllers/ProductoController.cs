...
namespace ESAP.Sirecec.Data.Api.Controllers
{
	...
	public class ProductoController : BaseController
	{
		...
		public ProductoController(DataContext context, IHttpContextAccessor httpContextAccessor)
		{
			_db = context;
		...
		public ActionResult Edit()
		{
			...
			if (item.Id == 0)
			{
				var obj = (Producto)item.CopyTo(new Producto());
				obj.CreadoPor = GetUserId();
				obj.CreadoEl = DateTime.Now;
				_db.Producto.Add(obj);
				_db.SaveChanges();
				return Ok(obj);
			}
			else
			{
				...
				var current = _db.Producto.First(o => o.Id == item.Id);
				if (current != null)
				{
					var final = (Producto)item.CopyTo(current);
