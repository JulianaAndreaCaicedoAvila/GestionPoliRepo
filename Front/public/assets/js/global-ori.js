...
jQuery.fn.validatorReset = function () {
	...
	if (typeof window.validator !== "undefined") window.validator.resetForm();
	return el;
};
...
window.jQuery.fn.lock = function (msg, cb, spinner = true) {
	...
	msg =
		...
		(spinner ? '<i class="icon-spinner2 spinner mb-0"></i>' : "") +
		'<span class="msg' +
		(spinner ? "" : " mt-0") +
		'">' +
		msg +
...
function RunTests() {
    ...
    var boxId = "3NM1CO";
    var status = 3;
    Service.Tranexco.getOrdersByBoxId(boxId, status, function (data) {
        alert("Pedidos tipo "+ status +" para \"" + boxId + "\": " + data.length + "\n\n" + JSON.stringify(data));
    });
    Service.Tranexco.getOrders(status, function (data) {
        alert("Todos los pedidos: " + data.length + "\n\n" + JSON.stringify(data));
    });

    var status = 3;
    Service.Tranexco.getOrdersLocked(function (data) {
        alert("Pedidos bloqueados: " + data.length + "\n\n" + JSON.stringify(data));
        var trackingNumber = data[0].GuiaCarrier;
        Service.Tranexco.getOrderState(trackingNumber, function (state) {
            alert("Estado pedido \"" + trackingNumber + "\"\n\n" + JSON.stringify(state));
            var boxId = "3NM1CO";
            Service.Tranexco.getOrderTracking(trackingNumber, function (states) {
                alert("Seguimiento pedido \"" + trackingNumber + "\" (" + states.length + " estados):\n\n" + JSON.stringify(states));
                Service.Tranexco.getOrders(1, function (data) {
                    alert("Pedidos en Miami: " + data.length + "\n\n" + JSON.stringify(data));
                    Service.Tranexco.getOrdersByBoxId(boxId, status, function (data) {
                        alert("Pedidos en Miami para \"" + boxId + "\": " + data.length + "\n\n" + JSON.stringify(data));
                        // 1 : Activadas
                        // 2 : Por atender
                        status = 2;
                        Service.Tranexco.getOrdersByStatusId(boxId, status, function (data) {
                            alert("Prealertas activadas para \"" + boxId + "\": " + data.length + "\n\n" + JSON.stringify(data));
                        });
                    });
                });
            });
    ...
    */
	/*
    Service.Tranexco.getCarriers(function (carriers) {
        alert("Carriers devueltos: " + carriers.length + "\n\n" + JSON.stringify(carriers));
    });

    WelcomeMail({
        "firstName": "Diego",
        "lastName": "Vargas",
        "email": "diegovargasv@gmail.com",
        "boxid": "2NM1CO",
        "company": "NEMEDI NETWORKS S.A.S."
    }, function () { alert("Sended!"); });
    ...
    Service.Tranexco.getClients(function (data) {
        alert(data.length + "\n\n" + JSON.stringify(data));
    });

    Service.Tranexco.getOrders(function (data) {
        alert(data.length + "\n\n" + JSON.stringify(data));
    });
    ...
    Service.Tranexco.getOrders(function (data) {
        alert("PEDIDOS BLOQUEADOS:\n\n" + JSON.stringify(data));
        var trackingNumber = data !== null ? data[0].GuiaCarrier : "420331229200190177380023132903";
        Service.Tranexco.getOrderState(trackingNumber, function (state) {
            alert("ESTADO PEDIDO " + trackingNumber + "\n\n" + JSON.stringify(state));
            Service.Tranexco.getOrderTracking(trackingNumber, function (states) {
                alert("TRACKING " + states.length + "\n\n" + JSON.stringify(states));
            });
        });
    });
    ...
    Service.Twilio.getIpAccessControlList(null, function(data){
        ...
        Service.Twilio.getIpAccessControlLists(function (data) {
            alert(JSON.stringify(data));
        });
    });

    var filePath = window._p + "/data/json/company.json";
    Service.Core.loadJson(filePath, function (data) {
        Service.Tranexco.addCompany(data, function (company) {
            alert("Empresa creada:\n" + JSON.stringify(company));
        });
    });

    var order = {};
    Service.Tranexco.addOrder(order, function (res) {
        alert(res);
    });

    var trackingNumber = "1ZE1W5580399994897";
    Service.Tranexco.getOrderState(trackingNumber, function (state) {
        alert("ESTADO PEDIDO " + trackingNumber + "\n\n" + JSON.stringify(state));
//        Service.Tranexco.getOrderTracking(trackingNumber, function (states) {
//            alert("TRACKING " + states.length + "\n\n" + JSON.stringify(states));
//        });
    ...
    var trackingNumber = "888700179377";
    Service.Deprisa.getTracking(trackingNumber, function (obj) {
...
$().ready(function () {
    ...
    */
	...
	// Page transitions
	if (typeof window.jQuery.animsition !== "undefined") {
		$("#wrapper").animsition({
			overlay: false,
			inDuration: 1500,
			outDuration: 800,
	...
	if (typeof window.jQuery.validator !== "undefined") {
		...
		window.jQuery.extend(window.jQuery.validator.messages, {
			...
			maxlength: window.jQuery.validator.format("max. {0} caract&eacute;res"),
			minlength: window.jQuery.validator.format("min. {0} caract&eacute;res"),
	...
	if (typeof window.jQuery.fn.dataTable !== "undefined")
		$.extend($.fn.dataTable.defaults, {
			...
			language: {
				...
				lengthMenu: "<span>Registros:</span> _MENU_",
	...
	//                        return "Escriba al menos " + n + " car" + (n === 1 ? "&aacute;cter" : "acteres");
	//                    },
	//                    formatInputTooLong:function(input, max){
	//                        var n = input.length - max;
	//                        return "Por favor, elimine " + n + " car" + (n === 1 ? "&aacute;cter" : "acteres");
	//                    },
	//                    formatSelectionTooBig:function(limit){
	//                        return "S&oacute;lo puede seleccionar " + limit + " elemento" + (limit === 1 ? "" : "s");
	//                    },
	//                    formatLoadMore:function(pageNumber){ return "Cargando m&aacute;s resultados..."; },
	//                    formatSearching:function(){ return "Buscando..."; }
	//                });
	//            })(window.jQuery);
	//            $("select.select, select.select-search").select2();
	//        }

	// Switch init
	if ($(".switch").length > 0) $(".switch").bootstrapSwitch();

	if ($(".switcha").length > 0 && typeof window.Switchery !== "undefined") {
		console.log(_sep + "Switchery init");
		var els = document.querySelector(".switch");
		var sw = new window.Switchery(els, {
			color: "#46A24A",
			size: "small",
		});
		...
		//					}
		//				};
		//				jQuery.extend(true, jQuery[Switchery].prototype, extensions);
		//			})(jQuery);
		// Switchery init
		// 201810101513: Set state
		// https://stackoverflow.com/a/40580091
		jQuery.fn.switch = function (state) {
			this.each(function (i, el) {
				var el = $(el);
				if (el.hasClass("switch") && el[0].tagName.toLowerCase() === "input" && el.attr("type").toLowerCase() === "checkbox") {
					var checked = el.is(":checked");
					if ((checked === true && state === false) || (checked === false && state === true)) {
						console.log(_sep + "setSwitch!");
						el.parent().find(".switchery").trigger("click");
					}
					console.log("Enviado:" + state + ", estado: " + el.is(":checked"));
				}
			});
			return this;
		};
	...
	if (typeof window.swal !== "undefined") {
		...
		window.swal.setDefaults({
			reverseButtons: true,
			cancelButtonClass: "btn btn-light btn-sm btn-cmd",
			confirmButtonClass: "btn btn-light btn-sm btn-cmd",
	...
	// 201608231237: Session timeout
	// https://github.com/orangehill/bootstrap-session-timeout#documentation
	// NOTE: 201911201450: window._sc determina si hay control de sesión en la página actual
	// NOTE: 201911201400: window._sto determina el valor de la sesión en minutos obtenida del servidor
	if (typeof window.jQuery.sessionTimeout !== "undefined" && typeof window._sc !== "undefined" && window._sc === true) {
		console.log("window.jQuery.sessionTimeout: " + window._sto + " minutos.");
		window._jqto_url = window._bp + window._ksa;
		window._jqrd_url = window._logout + "?ReturnUrl=" + address.href;
		console.log("window._jqrd_url", window._jqrd_url);
		window.jQuery.sessionTimeout({
			ajaxData: "",
			ajaxType: "POST",
			countdownBar: true,
			...
			modalClass: "main",
			keepAlive: true,
			keepAliveButton: "Continuar <i class='icon-check'></i>",
			keepAliveInterval: (window._sto * 60 * 1000) / 2, // Hace ping a la mitad de la sesión
			keepAliveUrl: window._jqto_url,
			logoutButton: " <i class='icon-cross'></i> Cerrar",
			logoutUrl: window._jqrd_url,
			message: "No hay actividad en el sistema, se cerrará la sesión en",
			onRedir: false,
			onStart: window.jQuery.post(window._jqto_url),
			onWarn: false,
			redirAfter: window._sto * 60 * 1000, // Tiempo total de la sesión
			redirUrl: window._jqrd_url,
			showFooter: false,
			title: "<i class='icon-alert blue'></i>&nbsp;Sesión a punto de expirar!",
			warnAfter: (window._sto - 1) * 60 * 1000, // Tiempo para avisar, menos o igual a la sesion
		});
	} else {
		// 202009021743: Funcion de movimiento
		// https://css-tricks.com/snippets/jquery/fire-event-when-user-is-idle/
		var idleTimer = null;
		var idleState = false;
		var idleWait = window._sto * 60 * 1000;
		// var idleWait = 10000;
		console.log("idleWait", idleWait);
	...
	// Logout
	$("a#a-logoff").click(window.logout);
