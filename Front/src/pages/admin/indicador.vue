...
import { useGeneralStore, useProductoStore, useAuthStore } from "@/stores";
import DxValidator, {
  DxRequiredRule,
  DxStringLengthRule,
...
const route = useRoute(),
  ...
  auth = useAuthStore();
let titulo = "Administración &raquo; Cursos &raquo; Indicadores",
  valGroup = ref(null),
  productos = ref([]),
  item = ref({
    id: 0,
  ...
  active = (data) => {
    ...
    msg.confirm({
      ...
      text: `¿Realmente desea ${data.activo ? "desactivar" : "activar"
        } el Indicador "<span class="font-weight-semibold">${data.nombre
        }</span>"?`,
      onConfirm: () => {
        panelGrid = $("#grid");
        panelGrid.lock(
...
onMounted(async () => {
  ...
  console.log(_sep);
  $("#grid").lock("Cargando");
  productos.value = await store.all();
    ...
    <div class="card data hidden" id="data">
      <div class="card-header main d-flex justify-content-between">
        <span>
          ...
          <span id="tit-action">Indicadores</span>
        </span>
      </div>
        ...
        <div class="card-body pt-3 pb-4">
          <div class="row">
            <div class="col-md-6 mb-1">
              <label class="tit">Objetivo</label>
              <DxSelectBox id="productoId" ref="productoIdTxtRef" :data-source="productos" :grouped="false"
                :min-search-length="3" :search-enabled="true" v-model="item.productoId" :show-clear-button="true"
                :show-data-before-search="true" class="form-control" @value-changed="itemSelected" placeholder="Objetivos"
            ...
            <div class="col-md-12 mb-2">
              ...
              <DxTextArea id="descripcion" value-change-event="keyup" :show-clear-button="true" v-model="item.descripcion"
          ...
          <div class="col">
            ...
            <DxDataGrid :customize-columns="customizeColumns" :data-source="dxStore" :hover-state-enabled="true"
              ...
              <DxPager :visible="true" :show-info="true" :show-page-size-selector="false" :show-navigation-buttons="true"
                :allowed-page-sizes="[15, 50, 'Todos']" info-text="{2} indicadores (página {0} de {1})" />
              <DxColumn data-field="productoId" caption="Productos" :visible="true" :allow-filtering="true">
                <DxLookup :data-source="productos" value-expr="id" display-expr="nombre" />
              </DxColumn>
              <DxColumn data-field="id" caption="Id" :visible="false" :width="80" :allow-filtering="false"
                :allow-sorting="true" alignment="center" />
              <DxColumn data-field="nombre" caption="Indicadores" :visible="true" />
              <DxColumn data-field="descripcion" caption="Descripcion" :visible="true" />
