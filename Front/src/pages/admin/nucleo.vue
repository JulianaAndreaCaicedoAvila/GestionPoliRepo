...
import { useGenericStore, useBancoStore, useAuthStore } from "@/stores";
import DxValidator, {
  DxRequiredRule,
  DxStringLengthRule,
...
let titulo = "Administración &raquo; Cursos &raquo; Núcleos",
  ...
  active = (data) => {
    ...
    msg.confirm({
      ...
      text: `¿Realmente desea ${data.activo ? "desactivar" : "activar"
        } el Núcleo "<span class="font-weight-semibold">${data.nombre}</span>"?`,
      onConfirm: () => {
        panelGrid = $("#grid");
        panelGrid.lock(
  ...
  save = async () => {
    ...
    if (!result.isValid) {
      ...
      panelData.lock(
        `${item.id == 0 ? "Creando" : "Actualizando"} Núcleos`,
        async function () {
          let dto = item.value;
          console.log("dto =>", dto);
        ...
        <div class="card-body pt-3 pb-4">
          <div class="row">
            <div class="col-md-9 mb-1">
              <label class="tit">Banco de programa:</label>
              <DxSelectBox id="bancoId" ref="bancoIdTxtRef" :data-source="bancos" :grouped="false" :min-search-length="3"
                :search-enabled="true" v-model="item.bancoId" :show-clear-button="true" :show-data-before-search="true"
                class="form-control" @value-changed="itemSelected" placeholder="Banco de programas" value-expr="id"
          ...
          <div class="col">
            ...
            <DxDataGrid :customize-columns="customizeColumns" :data-source="dxStore" :hover-state-enabled="true"
              ...
              <DxPager :visible="true" :show-info="true" :show-page-size-selector="false" :show-navigation-buttons="true"
                :allowed-page-sizes="[15, 50, 'Todos']" info-text="{2} núcleos (página {0} de {1})" />
