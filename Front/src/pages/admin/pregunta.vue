...
import { useGenericStore, useEncuestaStore, useAuthStore } from "@/stores";
import DxValidator, {
  DxRequiredRule,
  DxStringLengthRule,
  ...
  active = (data) => {
    ...
    msg.confirm({
      ...
      text: `¿Realmente desea ${data.activo ? "desactivar" : "activar"
        } la Pregunta "<span class="font-weight-semibold">${data.nombre
        }</span>"?`,
      onConfirm: () => {
        panelGrid = $("#grid");
        panelGrid.lock(
        ...
        <div class="card-body pt-3 pb-4">
          <div class="row">
            <div class="col-md-12 mb-2">
              ...
              <DxTextArea :height="110" :max-length="400" value-change-event="keyup" :show-clear-button="true" id="titulo"
                v-model="item.titulo" class="form-control" placeholder="Descripción">
                <DxValidator>
                  <DxRequiredRule />
                </DxValidator>
              </DxTextArea>
          ...
          <div class="col">
            ...
            <DxDataGrid :customize-columns="customizeColumns" :data-source="dxStore" :hover-state-enabled="true"
              ...
              <DxPager :visible="true" :show-info="true" :show-page-size-selector="false" :show-navigation-buttons="true"
                :allowed-page-sizes="[15, 50, 'Todos']" info-text="{2} preguntas (página {0} de {1})" />
              <!-- <DxColumn
              ...
              <DxColumn data-field="titulo" caption="Pregunta" :visible="true" />
