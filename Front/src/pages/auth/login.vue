...
let login = async () => {
  if (loginZone.isValid()) {
    ...
    loginZone
      .find(".card")
      .lock("Verificando credenciales,<br/>un momento por favor");
    let res = await authStore
      .do(["autenticar", email.value, password.value])
      .catch((error) => {
        // console.clear();
    ...
    if (typeof res != "undefined") {
      ...
      loginZone
        .find(".card")
        .lock(`Hola ${res.firstName}, ingresando!<br/>Un momento por favor`);
      setTimeout(function () {
      ...
      if (accounts.length > 0) {
        ...
        let res = await authStore.login(name, email).catch((error) => {
          ...
          msg.error(
            ...
            "Verifique credenciales e inténtelo nuevamente",
            function (params) {
              $("#txt-email").focus();
            }
          );
        ...
        if (typeof res != "undefined") {
          ...
          loginZone
            .find(".card")
            .lock(
              `Hola ${res.firstName}, ingresando!<br/>Un momento por favor`
            );
          setTimeout(function () {
            router.push(returnUrl);
          }, 1000);
...
let recover = async () => {
  if (recoverZone.isValid()) {
    recoverZone
      .find(".card")
      .lock("Recuperando contraseña,<br/>un momento por favor");
    let res = await authStore
      .do([
        "recuperar",
        {
...
let register = async () => {
  if (recoverZone.isValid()) {
    recoverZone
      .find(".card")
      .lock("Registrando cuenta,<br/>un momento por favor");
    let res = await authStore
      .do([
        "registrar",
        {
      ...
      .catch((error) => {
        ...
        let text =
          err.code == "DuplicateUserName"
            ? `Ya existe una cuenta registrada con el correo electrónico <strong>${email.value}</strong>.`
            : err.description;
...
let confirm = async () => {
  if (recoverZone.isValid()) {
    recoverZone
      .find(".card")
      .lock("Confirmando cuenta,<br/>un momento por favor");
    let res = await authStore
      .do([
        "registrar",
        {
...
let reset = async (endPoint) => {
  if (activateZone.isValid()) {
    ...
    console.log("dto =>", dto);
    let res = await authStore.activar(JSON.stringify(dto)).catch((error) => {
      console.error("error =>", error);
      ...
      let text =
        err.code == "InvalidToken"
          ? `El correo enviado para la ${
              endPoint == "activar"
                ? `activación de<br>la cuenta`
                : `recuperación de<br>la contraseña`
            } ha caducado o ya fue utilizado.`
          : err.description;
      msg.error(
        `Error al ${
          endPoint == "activar" ? `activar la cuenta` : `asignar la contraseña`
        }`,
        text,
        function (params) {
          router.push("ingreso");
    ...
    if (typeof res != "undefined") {
      console.log("res =>", res);
      let txt =
        endPoint == "activar" ? "Cuenta activada!" : "Contraseña asignada!";
...
/**
 * Esta zona es donde deffinimos los doifrentes metodos de uso
 */
</script>
<template>
  <div class="container py-4" id="main-zone">
      ...
      <div class="col d-flex align-items-center justify-content-center">
        <div class="card col-5">
          <div class="card-header main">
            <i class="fa-solid fa-user-lock"></i> Ingresar al sistema
          </div>
          <div class="card-body pt-3 pb-4" @keyup.enter="login">
            <div class="row">
              <div class="form-group">
                <label class="form-label color-main font-weight-semibold"
                  >Correo electrónico:
                  <span class="text-color-danger">*</span></label
                >
                <div class="form-icon position-relative">
                  <i
                    class="fa-solid fa-envelope text-4 color-main position-absolute left-15 top-50pct transform3dy-n50"
                  ></i>
                  <input
                    required
                    email
            ...
            <div class="row">
              <div class="form-group m-0">
                <label class="form-label color-main font-weight-semibold"
                  >Contraseña: <span class="text-color-danger">*</span></label
                >
                <div class="form-icon position-relative">
                  <i
                    class="fa-solid fa-key text-4 color-main position-absolute left-15 top-50pct transform3dy-n50"
                  ></i>
                  <input
                    required
                    minlength="5"
          ...
          <div class="card-footer main">
            <div class="d-flex justify-content-between">
              ...
              <button
                ...
                title="Ingresar"
              >
                INGRESAR <i class="fa-solid fa-caret-right ms-1"></i>
              </button>
            </div>
    ...
    <div class="row hidden" id="recover-zone">
      <div class="col d-flex align-items-center justify-content-center">
        <div class="card col-5">
          <div class="card-header main">
            <i class="fa-solid fa-arrow-rotate-left"></i> Recuperar contraseña
          </div>
          <div class="card-body pt-3 pb-2">
            <div class="row" v-if="action == 'register'">
              <div class="col">
                <div class="form-group">
                  <label class="form-label color-main font-weight-semibold"
                    >Nombres: <span class="text-color-danger">*</span></label
                  >
                  <div class="form-icon">
                    <input
                      required
              ...
              <div class="col">
                <div class="form-group">
                  <label class="form-label color-main font-weight-semibold"
                    >Apellidos: <span class="text-color-danger">*</span></label
                  >
                  <div class="form-icon">
                    <input
                      required
          ...
          <div class="card-footer main">
            <div class="d-flex justify-content-between">
              <button
                type="button"
                class="btn btn-gray"
                @click.prevent="recoverBack"
              >
                <i class="fa-solid fa-caret-left me-1"></i> INGRESAR AL SISTEMA
              </button>
              <button
                type="button"
                class="btn btn-main"
    ...
    <div class="row hidden" id="activate-zone">
      <div class="col d-flex align-items-center justify-content-center">
        <div class="card col-5">
          <div class="card-header main">
            <i class="fa-solid fa-user-plus"></i> Activar cuenta
          </div>
    ...
    <!-- Esta seccion es la de html donde def -->
  </div>
</template>
