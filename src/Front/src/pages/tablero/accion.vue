...
import { useClasificadorStore, useAccionStore } from "@/stores";
...
const accionStore = useAccionStore();
...
let areas = ref([]),
	...
	itemSelected = async (e) => {
		...
		// Carga las acciones por area
		acciones.value = await accionStore.cargar("area", areaId.value);
	},
	getHijos = (padreId) => {
		return objEspecificos.value.filter((o) => o.padreId == padreId);
	},
	getAcciones = (obEspId) => {
		return acciones.value.filter((o) => o.objetivoEspId == obEspId);
	},
	rdn = (rowData, e) => {
...
onMounted(async () => {
	...
	// Carga las acciones por area
	acciones.value = await accionStore.cargar("area", areaId.value);
	console.log("acciones =>", toRaw(acciones.value));
	objGenerales.value = await store.porPadre(areaId.value);
	...
	<div class="container pt-2 pb-2 content">
		<div class="card mb-4">
			<div class="card-header main">
				<div class="row">
					<div class="col-md-5 d-flex align-items-center"><i class="fa-solid fa-chart-simple"></i> Avances por acciones</div>
					<div class="col-md-7">
						<DxSelectBox
							id="objetivoId"
							:data-source="areas"
								...
								<div class="row m-0">
									<div class="col-md-12 fs">
										...
										<div class="row m-0 mb-0" v-for="item in getHijos(item.id)">
											...
											<div class="col-md-12 ps-5 bb mb-3 pb-1">
												<legend class="mb-1 pb-1 sub">Acciones</legend>
												<div class="row" v-for="item in getAcciones(item.id)">
													<div class="col-md-12 m-0 mb-1 d-flex justify-content-between align-items-start">
														<p class="m-0 p-0 me-5">
															<span class="font-weight-semibold" v-if="item.orden">{{ item.orden }}.</span>{{ item.nombre }}
														</p>
